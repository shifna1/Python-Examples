...
use movies;
select first_name,last_name,salary,job_id from employees where first_name like 's%';
select max(salary) from employees;
select * from employees where salary=24000;
select * from employees where salary =(select min(salary) from employees);
select count(*) as count,department_name from employees join departments on employees.department_id=departments.department_id group by departments.department_id; 
select count(*),e.manager_id,e.first_name,e.last_name from employees as e join employees as m on e.employee_id=m.manager_id group by m.manager_id;
